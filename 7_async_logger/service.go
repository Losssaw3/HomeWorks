...
// ///////////////////////////////////////
func UnaryAuthInterceptor(adm *adminService, acl map[string][]string) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp interface{}, err error) {
		...
		if !checkAccess(acl, consumer, info.FullMethod) {
			return nil, status.Error(codes.Unauthenticated, "no access")
		}

		remoteAddr, _ := peer.FromContext(ctx)
		event := Event{
			Timestamp: time.Now().Unix(),
			Consumer:  consumer,
			Method:    info.FullMethod,
			Host:      remoteAddr.Addr.String(),
		}
		adm.LogEvent(&event)
...
func StreamAuthInterceptor(adm *adminService, acl map[string][]string) grpc.StreamServerInterceptor {
...
func checkAccess(acl map[string][]string, user string, method string) bool {
	allowedMethods, ok := acl[user]
	if !ok {
		return false
	}
	for _, m := range allowedMethods {
		if method == m || (strings.HasSuffix(m, "*") && strings.HasPrefix(method, strings.TrimSuffix(m, "*"))) {
			return true
		}
	}
	return false
...
func (adm *adminService) LogEvent(event *Event) {
	adm.mu.Lock()
	defer adm.mu.Unlock()
	for _, ch := range adm.activeLoggers {
		select {
		case ch <- event:
		default:
		}
...
func (adm *adminService) WriteStat(event *Event) {
	adm.statMu.Lock()
	subs := append([]*statSub(nil), adm.activeStats...)
	adm.statMu.Unlock()

	for _, sub := range subs {
		sub.mu.Lock()
		sub.stat.ByConsumer[event.Consumer]++
		sub.stat.ByMethod[event.Method]++
		sub.mu.Unlock()
	}
}

func (adm *adminService) RemoveLogger(ch chan *Event) {
	adm.mu.Lock()
	defer adm.mu.Unlock()
	for i, c := range adm.activeLoggers {
		if c == ch {
			adm.activeLoggers = append(adm.activeLoggers[:i], adm.activeLoggers[i+1:]...)
			break
		}
	}
}

func (adm *adminService) Logging(nah *Nothing, stream Admin_LoggingServer) error {
	ch := make(chan *Event, 10)

	adm.mu.Lock()
	adm.activeLoggers = append(adm.activeLoggers, ch)
	adm.mu.Unlock()
...
func (adm *adminService) RemoveSub(sub *statSub) {
	adm.statMu.Lock()
	defer adm.statMu.Unlock()
	for i, s := range adm.activeStats {
		if s == sub {
			adm.activeStats = append(adm.activeStats[:i], adm.activeStats[i+1:]...)
			break
		}
	}
}

func (adm *adminService) Statistics(interval *StatInterval, stream Admin_StatisticsServer) error {

	sub := &statSub{mu: sync.Mutex{}, stat: &Stat{
		ByMethod:   make(map[string]uint64),
		ByConsumer: make(map[string]uint64),
	}}
	adm.statMu.Lock()
	adm.activeStats = append(adm.activeStats, sub)
	adm.statMu.Unlock()
...
func (*bizService) Check(ctx context.Context, nah *Nothing) (*Nothing, error) {
	return &Nothing{Dummy: nah.Dummy}, nil
}

func (*bizService) Add(ctx context.Context, nah *Nothing) (*Nothing, error) {
	return &Nothing{Dummy: nah.Dummy}, nil
}

func (*bizService) Test(ctx context.Context, nah *Nothing) (*Nothing, error) {
	return &Nothing{Dummy: nah.Dummy}, nil
}
...
func StartMyMicroservice(ctx context.Context, listenAddr string, ACLData string) error {
	...
	admin := &adminService{mu: sync.Mutex{}, activeLoggers: make([]chan *Event, 0), statMu: sync.Mutex{}, activeStats: []*statSub{}}
	server := grpc.NewServer(
		grpc.UnaryInterceptor(UnaryAuthInterceptor(admin, acl)),
		grpc.StreamInterceptor(StreamAuthInterceptor(admin, acl)),
	)
